...
**`arrayContaining` asserts presence, not exclusivity — it cannot see an extra violation.** WP15.4 found a validation bug that had been sitting in the suite unseen: the one test updating a scenario-less Leaf asserted its errors with `arrayContaining`, so a *second*, wrong violation alongside the expected one passed silently. Where a test is about *which* errors a thing produces, assert the exact set. Reserve `arrayContaining` for cases where you genuinely do not care what else is in the list — and notice that on an error assertion, you almost always do.

**A green test can be green because a *different* rule rejected the input.** WP15.1 mutation-checked a test named "rejects a diagram whose spec format is not one the renderer knows" and found it survived a mapper that laundered unknown formats away — because a *separate* schema rule refused a `spec` with no `specFormat`, and that was what had been doing the rejecting all along. The enum the test was named for was never load-bearing. Two consequences: **mutate the specific rule the test claims to be about**, not the general area, and if the test stays green, the assertion does not rest where its name says it does. The fix is usually a second case that strips away whatever else could reject the input — there, a fixture with no `spec` at all, leaving the enum as the only thing that could fail it.
...
**Any change to the content model requires a maximal-fixture contract test to pass** — author a record with *every* optional field populated, read it back through the real system, assert every field survives. WP15 shipped a mapper that dropped all three new fields with 932 tests green, because dropped optional fields are indistinguishable from absent ones.

> **⚠️ Corrected 2026-09-17, and read this before relying on the paragraph above.** This previously said *"One test in `apps/backend`, run against real Payload… fetch it through the backend."* **No such test exists and none ever has.** What exists is `apps/pipeline/tests/test_cms_roundtrip.py` — real Payload, `live`-marked, citing WP15 in its own docstring — but it round-trips **pipeline → Payload → pipeline and never calls the backend at all** (verified: zero references to port 3000). **So the seam WP15's defect actually lived on has no contract test**, which is how VO-1's audio-URL defect survived a green suite and had to be found by reading the mapper. Found by Manager asking instead of assuming. On the register; not yet packaged.
