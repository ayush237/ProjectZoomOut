...
**When you fix something, find its sibling.** Four of WP20's seven defects were a guard or fix that existed in one place and was never carried to its twin: the SDK retry fix in the text client but not the image client, find-then-skip idempotency in the CMS writer but not the asset path — where it then failed at Leaf 11 of 18, the exact failure that node's own comment describes — and the answer-length check in the retrofit command but not the graph. The codebase knew about all of them, in the wrong file. **Before closing anything, ask what the parallel implementation of this concern is, and check it.** Where a pair exists, both get tests.
