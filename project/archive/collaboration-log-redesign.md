# Collaboration Log — archive: the visual redesign and the first two books

**Split out of `project/collaboration-log.md` on 2026-09-15.** Covers WP15.7 through WP28.1 —
the eleven-package visual redesign (WP21–WP28.1), Track 42's publication, and the packages
either side of them. The active log keeps the four most recent of each section plus any handoff
whose package is still open; everything else lives here.

**This is the durable record.** Nothing was deleted — it is simply no longer loaded by every
session at startup. The active file was 417KB (~106k tokens) when this split was made, against a
40k target for the whole `project/` set.

**Cite entries here by commit hash, not by file position** — that rule exists because WP28 lost
time to a handoff that had been pruned out from under a citation.
...
**Every acceptance criterion is met, but not all by literally porting `tabs.jsx` — read the scope section before anything else, because it is the one decision here that is mine to explain but not mine to ratify.** Root `lint`, `typecheck` (4 workspaces), `test` (1,330 passing: shared 71, admin 198, backend 477, mobile 584 — 576 existing + 8 new) and `build` (backend/mobile/admin all produced real output) are clean from a genuine cold gate: `dist`/`.next` deleted across all four workspaces, `npm install` at the root, then all four commands fresh. Branched from `origin/main` (confirmed up to date, not the stale local `wp26-track-complete-share-card` branch — see "PR #39" below) as `wp27-tab-screens-icon-swap`; pushed, PR [#40](https://github.com/ayush237/ProjectZoomOut/pull/40).

**Time:** roughly 40% reading (handoff, `tabs.jsx`, both screen prompts, the current four screens, the component library, design tokens, `roadmapGeometry`/`roadmapModel`, and tracing the icon through its three consumers), 30% implementation, 15% device verification, 10% the cold gate, 5% this report.

---

**Scope: what "port from tabs.jsx" turned out to mean.** `tabs.jsx` is an interactive demo built to be clicked through, not a spec, and in three places it invents UI that either duplicates another tab's job or has no data behind it at all. screen-04.txt and screen-05.txt — the "unusually specific" written prompts the handoff asked me to read in full — are the more authoritative source for *what* each screen contains; `tabs.jsx` wins on visual/wiring detail where the two don't conflict. Where they diverge, here is what I built and what I didn't, and why:

- **Journey.** Kept the existing structure — a list of Tracks with somewhere to resume, each with a resume button — because that *is* screen-04's description ("books in progress, resume at the next incomplete Leaf"). Did **not** build `tabs.jsx`'s "Today" card (streak, XP, session-cap bar) or its "Next leaves" cross-track restructuring: neither is in the written spec, and the card's week-activity dots (M T W T F S S, done/today/ahead) have **exactly the same data problem the handoff named for the heatmap**. `tabs.jsx`'s own demo fakes those dots by inferring backward from a raw streak count assuming "today" is always the fifth day shown — `ReaderStanding`/`DayStatus` exposes `streak.current`, `streak.longest`, `streak.lastActiveLocalDate` (one date, not a history) and today's `session`, nothing else. Approximating a week of activity from a streak integer is the same shape of dishonesty as approximating a heatmap from one, so I didn't. The one change Journey *does* need — the compressed graph strip — is built; see below.
- **Library.** Kept the flat list. Did **not** build the "In progress / Saved / Finished" segmented control: "Saved" is a wishlist/bookmark concept that does not exist anywhere in `api/client.ts` or `packages/shared` — I grepped both for `saved`/`bookmark`/`wishlist` and found nothing, and confirmed `LibraryEntry.status` (`'active' | 'completed' | 'archived'`, marked `PROVISIONAL` in `packages/shared/src/user.ts` since WP7) is a different, unrelated, undocumented axis that nothing in the current UI reads at all. Building "Saved" would mean inventing either fake client-only state or new meaning for `archived` neither of us has specified. Added instead: a real `{count} tracks` header, matching `tabs.jsx`'s header treatment with real data.
- **Explore.** Kept the infinite-scroll catalogue (WP11's mechanism — already solid, already tested, already has a genuine end-of-list affordance). Did **not** build `tabs.jsx`'s "Pick up where you stopped" section (that's Journey's job, and screen-04 describes Explore as browsing only) or its search input / genre chips: `listTracks(page, perPage)` takes no query parameter, so either would mean faking a search over only the twenty *loaded* Tracks — which is a worse defect than no search, because it looks like it searches the whole catalogue and doesn't. Added instead, from real data only: the one-line description screen-04 explicitly asks for (`TrackCard`'s existing `children` slot — no change to `TrackCard.tsx` itself), and a header count (`"{list.length} of {totalTracks}"`) that answers the actual complaint screen-04 raises ("stops at twenty with no sign more exists") by telling the reader up front how much more there is.
- **Profile.** Restyled the achievement grid (see below); left the streak card, the account fields and sign-out alone — they were already built on the same tokens and components as everything else in this package, so there was nothing to change other than what the two data questions cover.

If Architect wants the fuller `tabs.jsx` fidelity anyway — the Today card, search/chips, Library segments — that's real, identifiable follow-up scope now, not something quietly dropped. I'd flag before building any of it that it either needs backend work (a real per-day activity read, a saved/wishlist concept) or a product ruling that a fabricated version is acceptable, which screen-04/05's own "do not invent metrics" and the founder's heatmap ruling both argue against.

---

**What changed, file by file:**
- **`Icon.tsx`** — `achievement: 'trophy'` → `'leaf'`. Ionicons is a closed vocabulary (screen-08: "the icon set is Ionicons and is not being replaced"), so this is the closest available glyph to "organic" rather than a literal render of the mockup's blob-and-mark; the actual blob half of that mark already exists (`AchievementUnlock`'s `badgeBlobPath`, WP25) and needed no change. A genuine bespoke illustration is a design asset for later, the same way `ShareCard`'s `MascotSlot` already documents itself as a placeholder.
- **`components/index.ts`** — exported `badgeBlobPath` (previously used only inside `AchievementUnlock.tsx`, in the same folder) so Profile could reuse it. The only touch outside the handoff's named scope; additive, no behaviour change to the existing export.
- **`ProfileScreen.tsx`** — `AchievementTile`'s unlocked state now renders the same organic blob + centred icon as the unlock celebration (new `AchievementGlyph`, reusing `badgeBlobPath` at 32pt vs. `AchievementUnlock`'s 44pt) instead of a bare `Icon`. Locked tiles are unchanged — deliberately: a blob is what a reader is told they *have*, and giving an unearned tile the same object would blur the locked/unlocked distinction rather than sharpen it. This duplicates `AchievementUnlock`'s composition at a second size; small enough (~15 lines) that extracting a shared `AchievementBadge(size)` felt like scope creep into a file (`AchievementUnlock.tsx`) the handoff doesn't list — noted for whoever next touches either.
- **`JourneyScreen.tsx`** — each row's `ProgressBar` replaced by `TrackProgressStrip` (new, colocated in this file): fetches the Track's Leaves itself (`api.listLeaves`), derives states via the existing `buildRoadmapModel`, and renders the crop (below) as a small `react-native-svg` graphic, with the same `"N of M complete"` caption `ProgressBar` used underneath — the graphic is additional, not a replacement for the accessible statement of progress. **This is a per-row request `LibraryScreen.tsx`'s own docblock warns against** ("the request explosion `TrackProgressSummary` exists to prevent") — I judged it acceptable because that warning is about the whole shelf, which can run to dozens of entries, and Journey's list is already filtered to Tracks with somewhere to resume, which in practice is a handful. Worth reconsidering if a reader's active list ever gets long. Degrades to the caption alone (no crash, no blocked row) if a row's Leaf fetch is slow or fails — exercised for real by the existing Journey tests, none of which stub the Leaves endpoint.
- **`ExploreScreen.tsx` / `LibraryScreen.tsx`** — header rows as described above.
- **`journeyProgressStrip.ts` (new)** — the pure geometry: `buildProgressStrip(trackId, states)` calls the real `layoutRoadmap` at the Track's real seed and states, then crops a 96pt vertical window centred on the Leaf the reader is up to (`selectFocusIndex`). Tier A, 7 tests, including one that asserts the returned geometry `toEqual`s a direct `layoutRoadmap` call at the same inputs — the acceptance criterion ("matches the roadmap's shape for the same Track") stated as an assertion rather than an intention. **Note on `constellationFragment`:** the testing-expectations line names it as an existing precedent to follow; it does not exist anywhere in this tree (see "PR #39" below), so this module follows `roadmapGeometry`/`roadmapModel`'s pattern directly instead. **Note on the geometry's floor:** `GRAPH`'s vertical padding alone (halo radius + axon reach) is 55pt each end, so even a single-Leaf Track is 110pt tall — always taller than the 96pt window. "Compressed" therefore means a crop, not a rescale, for every real Track; I state this in the module's docstring so the next person doesn't reach for a `Math.min` against a case that can't occur (I initially wrote one, and the test written to exercise it proved it dead — removed both).
- **`surfaces.test.tsx`** — three new tests: Explore's count header, Profile's achievement grid with real locked/unlocked data (the existing "renders Profile" smoke test uses an empty backend, so this was previously unexercised), both Tier B one happy path. `journeyProgressStrip.test.ts` is the Tier A module.

---

**The two data questions, answered:**
1. **The activity heatmap.** Not reachable from the client. `api/client.ts` and everything it imports from `packages/shared` expose `streak.current`, `streak.longest`, `streak.lastActiveLocalDate` (a single date, not a history) and today's `session` — no per-day array, no history endpoint. Confirmed by reading both files in full, not by absence of a grep hit alone. The backend has `daily_session` rows per the handoff's own note; surfacing them is a backend change (a new endpoint, most likely `GET /progress/history` or similar) that is out of this package's scope. Not approximated from the streak count. Not built.
2. **The whole-history constellation.** Per the handoff, "consider" rather than a requirement, and it doesn't fall out of what this package already has: the per-Track roadmap machinery (`buildRoadmapModel`, `layoutRoadmap`) is keyed on one Track's Leaves and progress, and aggregating leaf-states across every Track a reader has ever finished is a materially different computation, not a parameter change. Not built.

**A third instance of the same shape, found while implementing, not asked about in the handoff:** Journey's week-activity dots — see the Journey bullet above. Naming it here alongside the other two because it's the same underlying limitation (no per-day activity data) surfacing a third time, which seems worth knowing about as one gap rather than three independent surprises next time a screen wants "what did the reader do each day."

---

**Two findings that aren't mine to fix, but are worth knowing about:**

- **PR #39 ("WP26: Track complete screen and share card fragment") is still open, not merged**, despite the roadmap/log recording a 2026-09-10 sign-off. I confirmed this directly: `origin/main` has none of WP26's code — no new Track-complete screen, no changes to `ShareCard.tsx` beyond what WP9 originally shipped, no `constellationFragment` module anywhere in the tree (`git diff e4dac09..origin/main` touches only `apps/mobile/src/screens/track/` — all WP22.2, not WP26 — plus planning docs). The `layoutRoadmap(states, viewport, seed)` signature and `ShareCard.tsx` the handoff pointed me at are both genuinely on `main`, just courtesy of WP22.2 and WP9 respectively, not WP26. Nothing in WP27's actual requirements turned out to depend on WP26's unmerged code — the three icon consumers the handoff named are either pre-existing (`ShareCard`, `AchievementUnlock`) or built by this package (Profile's grid) — so I built on `main` as instructed and left the `wp26-track-complete-share-card` branch alone. Flagging because "WP26, merged" was asserted twice in the handoff and isn't true yet, and because if PR #39 lands later with its own `Icon.tsx` or `ShareCard.tsx` touch, it should be checked against this package rather than assumed compatible.
- **The Leaf player's edge controls (the header close button, the footer Next button) were unresponsive throughout this session's device testing**, independent of Reduce Motion: `xcrun simctl spawn <udid> defaults read com.apple.Accessibility ReduceMotionEnabled` returned `0` both before and after an explicit `write ... -bool NO` and a full app relaunch, yet Reanimated logged `"Reduced motion setting is enabled on this device"` on every one of those bundle loads regardless. A tap on a mid-screen text link (`Something wrong here? Report it`) worked immediately in the same session. This is `LeafPlayerScreen.tsx`, outside WP27's scope, so I didn't touch it — but it's the same bug the WP27 handoff already named as "the leading suspect for WP25's unresolved 'Next button won't tap,'" and this narrows it: the cause is not simply "Simulator's Reduce Motion setting is on," since the OS-level default plist says it's off. Something else — a stuck animated overlay that fails to size itself away, or Reanimated reading a different signal than `defaults` — is the more likely mechanism now. I could not complete a Leaf live in this session because of it; I verified the achievement-unlock and share-card-capture flows via "Wrap up today" on Journey instead, which reads today's summary without going through the player.

---

**Device gate, what I actually did:** Both themes, on-device (iPhone 16 Pro simulator) — Explore (with the count header and a description), Library (with the count header, a real finished Track), Journey empty state and populated state (with the strip, next + locked nodes both visible and correctly cropped), Profile's achievement grid (three unlocked, three locked, both themes). Scrolled Explore live past twenty (confirmed page two loads — reached "Placeholder Filler Track 21"). All three icon consumers verified with real data: Profile's grid, `AchievementUnlock`'s celebration (twice — "Full Circle" and "Called It a Day" both unlocked live via the wrap-up flow), and `ShareCard` — both the on-screen preview *and* the actual captured PNG, opened from the native share sheet rather than judged from the preview, per the acceptance criterion's own wording. Accessibility-max text size: checked once, confirmed it reproduces only the already-logged, deliberately-unfixed clipping defect (title/caption/button text clipped at the top) and nothing new from this package's changes — reverted immediately, not re-litigated, per the standing ruling. Did not get a clean before/after on Reduce Motion **on** specifically, for the reason above; this package added no new Reanimated motion of its own (the strip is a static SVG), so the risk this leaves unverified is confined to `AchievementUnlock`'s pre-existing entrance animation, not anything new here.

**Files touched:** `apps/mobile/src/components/Icon.tsx`, `apps/mobile/src/components/index.ts`, `apps/mobile/src/screens/{ExploreScreen,LibraryScreen,JourneyScreen,ProfileScreen}.tsx`, `apps/mobile/src/screens/journeyProgressStrip.{ts,test.ts}` (new), `apps/mobile/src/screens/surfaces.test.tsx`.

**Tests added/updated:** `journeyProgressStrip.test.ts` (7, Tier A — node-selection and the geometry-matches-`layoutRoadmap` assertion). Three new cases in `surfaces.test.tsx` (Tier B, one happy path each): Explore's count header, Profile's achievement grid against real locked/unlocked data. All existing Journey/Library/Explore/Profile tests in `surfaces.test.tsx` pass unmodified — the strip and header changes were built to not require touching their assertions.

**Assumptions made:** all stated above in the scope section — the `tabs.jsx`-vs-written-spec scope calls, the `leaf` glyph choice, the per-row Leaf fetch in Journey, exporting `badgeBlobPath` rather than leaving it private.

**Follow-ups / tech debt for Architect:**
1. A ruling on the scope section above — whether the restrained interpretation stands or the fuller `tabs.jsx` fidelity (Today card, search/chips, Library segments) becomes its own package, and if so, what it's allowed to invent versus what needs backend work first.
2. PR #39 — merge, close, or explain; WP27 doesn't block on it but the next package that touches `ShareCard.tsx`, `Icon.tsx` or the Leaf player should know it's still out there.
3. The Leaf-player tap bug — now narrowed (not simply Reduce Motion), still open.
4. `AchievementUnlock`'s badge composition is now duplicated at a second size in Profile's grid; worth a shared `AchievementBadge(size)` if a third consumer ever appears.
5. The three data gaps (heatmap, whole-history constellation, Journey's week dots) all trace to one missing capability — per-day activity, reachable server-side via `daily_session` but not yet exposed to the client. Worth deciding once rather than three times.

**With this landed, the redesign is complete per the written specs.** I'd call Explore, Library and Profile done as scoped. Journey I'd call done for what screen-04 actually asks for, but flag that it's visually the plainest of the four next to `tabs.jsx`'s version — if the founder looks at it side by side with the mockup and wants more of what's there, that's follow-up #1 above, not a defect in what shipped.

**Post-merge note, added while resolving the conflict this entry landed in against PR #39:** the "Two findings" section above says `constellationFragment` "does not exist anywhere in this tree" — true when written, against `origin/main` before WP26 merged. It exists now (below): WP26 shipped it as `apps/mobile/src/screens/share/constellationFragment.ts`. I checked its consumer, `TrackCompleteScreen.tsx`, for a fourth surface rendering the achievement icon while resolving this conflict — it overrides `ShareCard`'s mascot slot with the constellation fragment itself rather than the default icon, so the three-consumer icon-swap verification above still stands; nothing new to re-check.
