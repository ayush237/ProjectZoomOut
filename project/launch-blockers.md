...
|---|---|---|---|---|
| 1 | **Track 42's cover hotlinks a retailer's CDN.** ~~Purchase URL has no scheme~~ ✅ **fixed 2026-09-02** — it was hiding the whole book, not a dead link. **The disclaimer was never broken; that claim was wrong** | needs a hosted asset | **The onboarding flow's first-book picker, as of 2026-09-18.** Today a cover is one card in a list; that screen makes covers the entire visual — two or three full-bleed cards and nothing else — so **this stops being cosmetic the moment onboarding is built.** **When it is fixed, the replacement lands as a relative Payload URL — which WP15.8 must already handle**, or the Track breaks again | 2026-09-01 |
...
### Moved here from the debt register — 2026-09-22

**The roadmap declares this file the single live list for founder decisions "and nowhere else," and the debt
register was quietly keeping a second one.** These six rows are decisions only the founder can make; they were
sitting in a technical-debt table where they read as engineering work. Moved verbatim, in the register's own
four-column shape rather than forced into Part 3's table, because reshaping them would mean rewriting them.

| Item | Impact | Flagged by | Status |
|---|---|---|---|
| **One founder check, ~60 seconds, closes WP28.1's only unobserved criterion** | **Open the Track roadmap in the disagreeing state and watch `NextNodeRing`.** WP28.1's criterion was *"observed in the disagreeing state: `NextNodeRing` animates"*, and **the observation was not decisive** — Manager said so plainly rather than rounding it up, which is the right call and is why this is a line rather than a rejection. **The negative half is decisive and is the harder half**: pre-fix, three screenshots spanning 3+ seconds were pixel-identical, so the bug is reproduced, not hypothesised. The positive half rests on the override being the identical mechanism WP28 already device-verified on the fade branch, plus mutation-checked structural presence. **A human distinguishes a pulsing ring from a still one instantly**; sequential screenshots cannot. **Procedure matters — set Reduce Motion ON, cold-launch, then flip it OFF without relaunching.** A relaunch clears the window and shows a passing app that proves nothing. **This is the same shape as the check that sat open four weeks and was shut in one minute on 2026-09-10** | Architect, WP28.1 sign-off, 2026-09-11 | Open — **founder**, ~60 seconds |
| **The roadmap is a graph on a page, not the page — full-bleed and gradient scrims are the largest remaining visual gap** | Both live outside `screens/track/` (`Screen.tsx`, `TrackDetailScreen.tsx`), so WP22.2 correctly left them. Manager's phrasing is the useful one: it is the difference between *a graph on a page* and the mockup's *graph as the page* | Manager, WP22.2 | Open — small follow-up package if the founder still sees a gap after the merge |
| **The roadmap's fourth node state, "revisit", has no data behind it and was deliberately not built** | The design mockup's fixture carries `REVISIT=[2,5]`, implying a state for Leaves worth returning to. **Nothing in the domain model or the API can supply it** — building it in WP22 would have meant inventing reader state to satisfy a picture. Plausible real meanings exist (a Leaf answered wrong on the first try is the obvious one — `leaf_progress.attempt_count` holds that server-side and no endpoint exposes it), but choosing one is a product decision, not an implementation detail | Architect, WP22 planning, 2026-09-08 | Open — **needs a founder decision on what "revisit" means before any package builds it.** Not blocking: the graph reads correctly with three states |
