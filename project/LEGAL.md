...
**How the two doors are held, as of 2026-09-25 (ONBOARD-2.1).** The greeting door now **refuses at runtime**, not only by test: `synthesize_greeting` compares the greeting it is handed with `NARRATOR_GREETINGS` — on both the stored words and the words sent — and raises `SpeechFenceError` before any request is built, so `dataclasses.replace(greeting, text=…)` and `object.__setattr__` cannot reach the voice. The error names the narrator and never repeats the words: a source quote in an error message is a source quote in a log. **The Leaf door has no such list to compare against** — a Leaf's line is whatever the Leaf says — so it is held **by tests alone**: the signature pin, a `NarrationLine` built in exactly one place in any shape, no `replace(…, text=…)` anywhere in `src/`, and the private `SpeechClient._call`, which will speak any string, reached only from the two doors. **Named, not closed:** dynamic access to `_call` (`getattr`), and aliasing or `**fields` on a `NarrationLine`, are invisible to a static test. Each pin was seen red against a scratch bypass while the old pins stayed green on the same one, which is why the new pins are additions and not edits. Nothing here relaxes the rule above: the tests may be strengthened and never loosened.
