...
| WP2.1 | Schema-freeze alignment (4 gate rulings) + small backend gaps (logout, error split, token reaping) | Manager | 🔵 Handed off 2026-08-08 | — |
| WP3 | Content API — Explore, Library, Track/Leaf delivery | Manager | 🔒 handoff written, **released after WP2.1** | WP2.1 — it maps and validates against the schemas WP2.1 changes |
...
| **Report gate timing** — how long one Leaf actually took vs the 45-min estimate | minutes | Whether the content plan is realistic at ~20 Leaves | ⬜ |
| **Rule on save-vs-publish authoring UX** — did the exactly-one-correct refusal help or interrupt? | minutes | Folds into WP2.1 free if answered now | ⬜ |
...
| "Exactly one correct option" fires on **every save**, not only on publish — a scenario cannot be saved between adding options and ticking one | Possible authoring friction. Built as the WP1 handoff specified | Manager, WP1 | **Deferred by decision** — decide during the schema-freeze gate, with real editing evidence. Architect's prior: move it behind publish |
